...
def SIS_linf_cost(q, w, h, B, b, cost_svp=svp_classical, verbose=False):
    ...
    log2_eps = d * log(p_middle, 2) + i * log(p_head, 2) #log de la proba que les i premieres coordonnees sont dans [-B,B] + log de la proba que les j-i suivantes sont dans [-B,B] aussi
    log2_R = max(0, - log2_eps - nvec_sieve(b)) #Probability that everything is in [-B,B]?
